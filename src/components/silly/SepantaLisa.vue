...
const timer = ref()
const resizeHandler = () => {
  box.value && box.value.style.setProperty('--duration', '0s')
...
onMounted(() => {
  document.querySelector('.app-page')?.addEventListener('scroll', scrollHandler)
  window.addEventListener('resize', resizeHandler)
})
onUnmounted(() => {
  document.querySelector('.app-page')?.removeEventListener('scroll', scrollHandler)
